<?php
/**
 * Admin: settings screen under Settings → Fuzzify Search, and the per-item exclude box.
 *
...
final class Admin {
	...
	public static function menu(): void {
		add_options_page(
			__( 'Fuzzify Search', 'fuzzify-search' ),
			__( 'Fuzzify Search', 'fuzzify-search' ),
			'manage_options',
			self::PAGE,
	...
	public static function rank_labels(): array {
		return array(
			'title'   => __( 'Title', 'fuzzify-search' ),
			'terms'   => __( 'Categories and tags', 'fuzzify-search' ),
			'sku'     => __( 'SKU', 'fuzzify-search' ),
			'excerpt' => __( 'Excerpt', 'fuzzify-search' ),
			'content' => __( 'Body text', 'fuzzify-search' ),
		);
	}
	...
	public static function preview_config(): array {
		return array(
			...
			'i18n'     => array(
				'prompt'      => __( 'Type a word or two to see how results would be ranked.', 'fuzzify-search' ),
				'loading'     => __( 'Loading the index…', 'fuzzify-search' ),
				'unavailable' => __( 'The index could not be loaded. Rebuild it first.', 'fuzzify-search' ),
				'none'        => __( 'No results.', 'fuzzify-search' ),
				'noPages'     => __( 'No pages match.', 'fuzzify-search' ),
				'one'         => __( '1 result', 'fuzzify-search' ),
				/* translators: %d: number of results */
				'many'        => __( '%d results', 'fuzzify-search' ),
				/* translators: 1: number shown, 2: total number of results */
				'showing'     => __( 'Showing the first %1$d of %2$d results', 'fuzzify-search' ),
				/* translators: 1: item name, 2: its new position, 3: number of items */
				'moved'       => __( 'Moved %1$s to position %2$d of %3$d.', 'fuzzify-search' ),
			),
		);
	}
	...
	public static function action_links( array $links ): array {
		...
		array_unshift( $links, '<a href="' . esc_url( $url ) . '">' . esc_html__( 'Settings', 'fuzzify-search' ) . '</a>' );
		return $links;
	}
	...
	public static function handle_rebuild(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have permission to do this.', 'fuzzify-search' ), '', array( 'response' => 403 ) );
		}
	...
	public static function render(): void {
		...
		<div class="wrap static-search-admin">
			<h1><?php esc_html_e( 'Fuzzify Search', 'fuzzify-search' ); ?></h1>
			<p class="description"><?php esc_html_e( 'Search that runs in the visitor’s browser from one index file, so it works on this site and in static HTML exports.', 'fuzzify-search' ); ?></p>
			<?php
			settings_errors();
			self::render_notices();
			self::render_status();
	...
	private static function render_notices(): void {
		...
		if ( isset( $_GET['static-search-rebuilt'] ) ) {
			...
			$message = sprintf( _n( 'Index rebuilt: %s item.', 'Index rebuilt: %s items.', $count, 'fuzzify-search' ), number_format_i18n( $count ) );
			echo '<div class="notice notice-success is-dismissible"><p>' . esc_html( $message ) . '</p></div>';
		}
		if ( isset( $_GET['static-search-error'] ) ) {
			// Fixed wording only. The reason is stored with the index status and listed in the warnings below.
			echo '<div class="notice notice-error"><p>' . esc_html__( 'The index could not be rebuilt. The reason is listed below.', 'fuzzify-search' ) . '</p></div>';
		}
		// phpcs:enable
	...
	private static function warnings(): array {
		...
		if ( '' !== $status['error'] ) {
			...
			$warnings[] = sprintf( __( 'The last index build failed: %1$s (%2$s ago)', 'fuzzify-search' ), esc_html( $status['error'] ), esc_html( human_time_diff( $status['error_time'] ) ) );
		}
		if ( ! Index::exists() ) {
			$warnings[] = esc_html__( 'There is no index file yet, so visitors still get the normal WordPress search. Press “Rebuild index now”.', 'fuzzify-search' );
		}
		if ( ! get_option( 'permalink_structure' ) ) {
			$warnings[] = esc_html__( 'Pretty permalinks are off. The results page needs them to work in a static export.', 'fuzzify-search' );
		}
		if ( Settings::get( 'results_enabled' ) && '' === Frontend::results_page_url() ) {
			$warnings[] = esc_html__( 'No results page is set. Pressing Enter in a search box falls back to the normal WordPress results page, which a static export cannot provide.', 'fuzzify-search' );
		}
		if ( $status['bytes'] > self::LARGE_INDEX ) {
			/* translators: %s: file size */
			$warnings[] = sprintf( __( 'The index is %s. Every visitor downloads it the first time they search; consider setting a content length limit below.', 'fuzzify-search' ), esc_html( size_format( $status['bytes'] ) ) );
		}
		if ( self::searchwp_live_active() ) {
			$warnings[] = esc_html__( 'SearchWP Live Ajax Search is active. Fuzzify Search leaves the search fields it has already taken over alone; deactivate it to use Fuzzify Search everywhere.', 'fuzzify-search' );
		}
	...
	private static function render_status(): void {
		...
		if ( ! Index::exists() ) {
			$state = __( 'No index file', 'fuzzify-search' );
		} elseif ( $dirty > $status['generated'] ) {
			$state = Settings::get( 'rebuild_on_save' )
				? __( 'Content changed since the last build; it will be rebuilt shortly.', 'fuzzify-search' )
				: __( 'Content changed since the last build. Rebuild to include the changes.', 'fuzzify-search' );
		} else {
			$state = __( 'Up to date', 'fuzzify-search' );
		}

		$page_id = (int) Settings::get( 'results_page' );
		?>
		<h2><?php esc_html_e( 'Index', 'fuzzify-search' ); ?></h2>
		<table class="widefat striped static-search-status" role="presentation">
			<tbody>
				<tr>
					<th scope="row"><?php esc_html_e( 'Items indexed', 'fuzzify-search' ); ?></th>
					<td><?php echo esc_html( number_format_i18n( $status['count'] ) ); ?></td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Index file', 'fuzzify-search' ); ?></th>
					<td>
						<?php if ( Index::exists() ) : ?>
							...
							printf( ' &middot; %s', esc_html( sprintf( __( '%1$s (about %2$s compressed)', 'fuzzify-search' ), size_format( $status['bytes'] ), size_format( max( 1, $status['gzip'] ) ) ) ) );
							?>
						<?php else : ?>
							&mdash;
						<?php endif; ?>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Last built', 'fuzzify-search' ); ?></th>
					<td>
						<?php
						if ( $status['generated'] ) {
							/* translators: 1: time difference, 2: date and time */
							echo esc_html( sprintf( __( '%1$s ago (%2$s)', 'fuzzify-search' ), human_time_diff( $status['generated'] ), wp_date( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), $status['generated'] ) ) );
						} else {
							echo '&mdash;';
						}
						?>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'State', 'fuzzify-search' ); ?></th>
					<td><?php echo esc_html( $state ); ?></td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Results page', 'fuzzify-search' ); ?></th>
					<td>
						<?php if ( ! Settings::get( 'results_enabled' ) ) : ?>
							<?php esc_html_e( 'Off: pressing Enter does nothing', 'fuzzify-search' ); ?>
		...
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="static-search-rebuild">
			...
			<?php submit_button( __( 'Rebuild index now', 'fuzzify-search' ), 'secondary', 'submit', false ); ?>
		</form>
		<?php
	}
	...
	private static function render_form(): void {
		...
		$fields   = array(
			'excerpt' => __( 'Excerpt (product short description)', 'fuzzify-search' ),
			'content' => __( 'Content', 'fuzzify-search' ),
			'terms'   => __( 'Categories, tags and other taxonomy terms', 'fuzzify-search' ),
			'sku'     => __( 'Product SKU', 'fuzzify-search' ),
		);
		?>
		<form method="post" action="options.php">
			<?php settings_fields( self::GROUP ); ?>

			<h2><?php esc_html_e( 'What to search', 'fuzzify-search' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><?php esc_html_e( 'Post types', 'fuzzify-search' ); ?></th>
					<td>
						<fieldset>
						...
						<p class="description"><?php esc_html_e( 'Only published items are indexed.', 'fuzzify-search' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Fields to search', 'fuzzify-search' ); ?></th>
					<td>
						<fieldset>
							<input type="hidden" name="<?php echo esc_attr( $name ); ?>[fields][]" value="">
							<label><input type="checkbox" checked disabled> <?php esc_html_e( 'Title (always)', 'fuzzify-search' ); ?></label><br>
				...
				<tr>
					<th scope="row"><label for="static-search-content-limit"><?php esc_html_e( 'Content length limit', 'fuzzify-search' ); ?></label></th>
					<td>
						...
						<?php esc_html_e( 'characters per item (0 = no limit)', 'fuzzify-search' ); ?>
						<p class="description"><?php esc_html_e( 'A limit keeps the index small on sites with a lot of long content.', 'fuzzify-search' ); ?></p>
					</td>
				</tr>
				<?php if ( function_exists( 'wc_get_page_id' ) ) : ?>
					<tr>
						<th scope="row"><?php esc_html_e( 'WooCommerce', 'fuzzify-search' ); ?></th>
						<td>
							<?php self::checkbox( $name . '[skip_woo_pages]', (bool) $settings['skip_woo_pages'], __( 'Leave out the Cart, Checkout and My account pages', 'fuzzify-search' ) ); ?>
							<p class="description"><?php esc_html_e( 'Products set to hidden or “shop only” are always left out, as in WooCommerce’s own search.', 'fuzzify-search' ); ?></p>
						</td>
					</tr>
				<?php endif; ?>
			</table>
			...
			<table class="form-table" role="presentation">
				<tr>
					...
					<td>
						...
						<?php esc_html_e( 'characters', 'fuzzify-search' ); ?>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="static-search-delay"><?php esc_html_e( 'Wait after typing', 'fuzzify-search' ); ?></label></th>
					<td>
						...
						<?php esc_html_e( 'milliseconds', 'fuzzify-search' ); ?>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="static-search-max-results"><?php esc_html_e( 'Results in the dropdown', 'fuzzify-search' ); ?></label></th>
				...
				<tr>
					<th scope="row"><label for="static-search-threshold"><?php esc_html_e( 'Typo tolerance', 'fuzzify-search' ); ?></label></th>
					<td>
						<select id="static-search-threshold" name="<?php echo esc_attr( $name ); ?>[threshold]">
							<option value="0.2" <?php selected( (float) $settings['threshold'], 0.2 ); ?>><?php esc_html_e( 'Strict: close matches only', 'fuzzify-search' ); ?></option>
							<option value="0.3" <?php selected( (float) $settings['threshold'], 0.3 ); ?>><?php esc_html_e( 'Balanced (recommended)', 'fuzzify-search' ); ?></option>
							<option value="0.4" <?php selected( (float) $settings['threshold'], 0.4 ); ?>><?php esc_html_e( 'Loose: more, less exact results', 'fuzzify-search' ); ?></option>
						</select>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Thumbnails', 'fuzzify-search' ); ?></th>
					<td><?php self::checkbox( $name . '[show_thumbs]', (bool) $settings['show_thumbs'], __( 'Show a thumbnail next to each result', 'fuzzify-search' ) ); ?></td>
				</tr>
				<tr>
			...
			<h2><?php esc_html_e( 'Results page', 'fuzzify-search' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><?php esc_html_e( 'Activate', 'fuzzify-search' ); ?></th>
					<td>
						<?php self::checkbox( $name . '[results_enabled]', (bool) $settings['results_enabled'], __( 'Send Enter to the results page', 'fuzzify-search' ) ); ?>
						<p class="description"><?php esc_html_e( 'When unticked, pressing Enter in a search box does nothing. The dropdown of instant results still works, and a highlighted result still opens on Enter, but the dropdown then shows at most the number set under Search box, with no “View all” link.', 'fuzzify-search' ); ?></p>
					</td>
				</tr>
				<tr data-results-dependent>
					<th scope="row"><label for="static-search-results-page"><?php esc_html_e( 'Page', 'fuzzify-search' ); ?></label></th>
					<td>
						<?php
						wp_dropdown_pages(
							array(
								'name'              => esc_attr( $name . '[results_page]' ),
								'id'                => 'static-search-results-page',
								'selected'          => (int) $settings['results_page'],
								'show_option_none'  => esc_html__( '— None (normal WordPress results) —', 'fuzzify-search' ),
								'option_none_value' => '0',
							)
						);
						?>
						<p class="description">
							...
							printf(
								...
								esc_html__( 'Where pressing Enter in a search box goes. The page must contain the %s shortcode; the one created on activation does.', 'fuzzify-search' ),
				...
				<tr data-results-dependent>
					<th scope="row"><label for="static-search-per-page"><?php esc_html_e( 'Results per page', 'fuzzify-search' ); ?></label></th>
				...
				<tr data-results-dependent>
					<th scope="row"><?php esc_html_e( 'Old search addresses', 'fuzzify-search' ); ?></th>
					<td>
						<?php self::checkbox( $name . '[forward_old_search]', (bool) $settings['forward_old_search'], __( 'Send /?s=term addresses to the results page', 'fuzzify-search' ) ); ?>
						<p class="description"><?php esc_html_e( 'A static site cannot answer /?s=term: it serves the plain page. With this on, anything that still links to such an address (bookmarks, other sites) is forwarded to the results page. On a live WordPress site, WordPress’ own results page is left alone.', 'fuzzify-search' ); ?></p>
					</td>
				</tr>
			</table>

			<h2><?php esc_html_e( 'Keeping the index current', 'fuzzify-search' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><?php esc_html_e( 'Automatic rebuild', 'fuzzify-search' ); ?></th>
					<td>
						<?php self::checkbox( $name . '[rebuild_on_save]', (bool) $settings['rebuild_on_save'], __( 'Rebuild shortly after content is published, changed or removed', 'fuzzify-search' ) ); ?>
						<p class="description"><?php esc_html_e( 'The index is also rebuilt when you save these settings and at the start of every Simply Static export.', 'fuzzify-search' ); ?></p>
					</td>
				</tr>
			</table>
					...
					<td>
						...
						<fieldset>
							<label>
								...
								<?php esc_html_e( 'Only between equally good matches (recommended)', 'fuzzify-search' ); ?>
							</label><br>
							<label>
								<input type="radio" name="<?php echo esc_attr( $name ); ?>[type_mode]" value="first" <?php checked( 'first' === $settings['type_mode'] ); ?>>
								<?php esc_html_e( 'Before everything else', 'fuzzify-search' ); ?>
							</label>
						</fieldset>
						<p class="description"><?php esc_html_e( 'Drag to set the order. By default every type shares one priority, which means no preference. With the first choice, a Product and a Post that match equally well are ordered by this list. With the second, every result of the top type comes before any of the next, whatever the field.', 'fuzzify-search' ); ?></p>
					</td>
				</tr>
			<?php endif; ?>
			<tr>
						...
						<table class="widefat striped" data-preview-table hidden>
							<thead>
								<tr>
									...
									<th scope="col"><?php esc_html_e( 'Result', 'fuzzify-search' ); ?></th>
									<th scope="col"><?php esc_html_e( 'Type', 'fuzzify-search' ); ?></th>
									<th scope="col"><?php esc_html_e( 'Matched in', 'fuzzify-search' ); ?></th>
								</tr>
							</thead>
							<tbody></tbody>
						</table>
						<p class="description"><?php esc_html_e( 'Uses the index as last built. Ranking changes show here at once, before you save; changes to what is indexed (post types, fields) show after you save.', 'fuzzify-search' ); ?></p>
					</div>
				</td>
			</tr>
		</table>
			...
			<ol class="static-search-sortable__list" role="list" data-rank-list>
				<?php foreach ( $items as $slug => $label ) : ?>
					...
					<li class="static-search-sortable__item<?php echo $tied ? ' is-tied' : ''; ?>">
						<span class="static-search-sortable__handle" data-handle aria-hidden="true" title="<?php esc_attr_e( 'Drag to reorder', 'fuzzify-search' ); ?>"></span>
						...
						<label class="static-search-sortable__tie">
							...
							<?php esc_html_e( 'Same priority as above', 'fuzzify-search' ); ?>
						</label>
						<span class="static-search-sortable__moves">
							<button type="button" class="button-link" data-move="up" aria-label="<?php /* translators: %s: item name */ echo esc_attr( sprintf( __( 'Move %s up', 'fuzzify-search' ), $label ) ); ?>"><span class="dashicons dashicons-arrow-up-alt2" aria-hidden="true"></span></button>
							<button type="button" class="button-link" data-move="down" aria-label="<?php /* translators: %s: item name */ echo esc_attr( sprintf( __( 'Move %s down', 'fuzzify-search' ), $label ) ); ?>"><span class="dashicons dashicons-arrow-down-alt2" aria-hidden="true"></span></button>
						</span>
	...
	private static function render_excluded(): void {
		$query = new \WP_Query(
			array(
				...
				'meta_key'       => Index::META_EXCLUDE, // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key -- admin-only list of hidden items, one key, capped at 200 rows.
				'meta_value'     => '1', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_value -- same query as above.
				'posts_per_page' => 200,
				'orderby'        => 'title',
				'order'          => 'ASC',
				'no_found_rows'  => true,
		...
		?>
		<h2><?php esc_html_e( 'Hidden from search', 'fuzzify-search' ); ?></h2>
		<p class="description"><?php esc_html_e( 'To hide an item, tick “Hide this item from search” in the Fuzzify Search box on its edit screen.', 'fuzzify-search' ); ?></p>
		<?php if ( ! $query->have_posts() ) : ?>
			<p><?php esc_html_e( 'No items are hidden.', 'fuzzify-search' ); ?></p>
		<?php else : ?>
	...
	public static function add_meta_box(): void {
		...
		if ( $types ) {
			add_meta_box( 'static-search-exclude', __( 'Fuzzify Search', 'fuzzify-search' ), array( self::class, 'render_meta_box' ), $types, 'side', 'default' );
		}
	}
	...
	public static function render_meta_box( \WP_Post $post ): void {
		...
		<label>
			...
			<?php esc_html_e( 'Hide this item from search', 'fuzzify-search' ); ?>
		</label>
		<?php
	}
	...
	public static function post_states( array $states, \WP_Post $post ): array {
		if ( Index::is_flagged( $post->ID ) ) {
			$states['static_search_hidden'] = __( 'Hidden from search', 'fuzzify-search' );
		}
		return $states;
	}
}
